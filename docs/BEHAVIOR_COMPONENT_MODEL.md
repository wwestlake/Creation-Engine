...
## 7. Delivery order (proposed, not started)

1. Structured control-flow lowering in `frust_codegen.cpp` for
   Branch/Sequence/For/While/Break/Continue/Return.
2. Save/load a Behavior graph as a named, reopenable asset (mirrors
   Material's `savedGraphSource` pattern).
3. Compile → produce a real loadable pod via the FRust compiler
   toolchain, cached, not regenerated by the running game.
